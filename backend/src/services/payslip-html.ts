/**
 * A payslip as a self-contained HTML document — the same markup the dashboard
 * prints and the app turns into a PDF. Earnings are shown at the full monthly
 * rate and loss of pay is a deduction line, so what the attendance rules cost
 * is on the slip rather than folded silently into a smaller earned figure.
 */
import { DEDUCTION_TRIGGER_LABELS, DeductionTrigger } from '../models/deduction.model';

export interface PayslipForHtml {
  employeeName: string;
  employeeId?: string;
  designation?: string;
  department?: string;
  joiningDate?: string;
  period: string;
  earnings: { name: string; fullPaise: number; paidPaise: number }[];
  deductions: { name: string; amountPaise: number }[];
  ...
  inputs: {
    ...
    attendanceDeductions?: { label: string; count: number; days: number; trigger?: string }[];
...
const monthLabel = (period: string) => new Date(`${period}-01T00:00:00Z`).toLocaleString('en-IN', { month: 'long', year: 'numeric', timeZone: 'UTC' });
const dateLabel = (iso?: string) => (iso ? new Date(`${iso.slice(0, 10)}T00:00:00Z`).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric', timeZone: 'UTC' }) : '—');
const lastDayOf = (period: string) => { const [y, m] = period.split('-').map(Number); return new Date(Date.UTC(y, m, 0)).toISOString().slice(0, 10); };

const ONES = ['', 'One', 'Two', 'Three', 'Four', 'Five', 'Six', 'Seven', 'Eight', 'Nine', 'Ten', 'Eleven', 'Twelve', 'Thirteen', 'Fourteen', 'Fifteen', 'Sixteen', 'Seventeen', 'Eighteen', 'Nineteen'];
const TENS = ['', '', 'Twenty', 'Thirty', 'Forty', 'Fifty', 'Sixty', 'Seventy', 'Eighty', 'Ninety'];
const belowHundred = (n: number) => (n < 20 ? ONES[n] : `${TENS[Math.floor(n / 10)]}${n % 10 ? `-${ONES[n % 10]}` : ''}`);
const belowThousand = (n: number) => [Math.floor(n / 100) ? `${ONES[Math.floor(n / 100)]} Hundred` : '', n % 100 ? belowHundred(n % 100) : ''].filter(Boolean).join(' ');
...
export function payslipHtml(slip: PayslipForHtml, company: { name: string; address: string }): string {
  ...
  const nameOf = (l: (typeof lines)[number]) => (l.trigger ? DEDUCTION_TRIGGER_LABELS[l.trigger as DeductionTrigger] ?? l.label : l.label);
  ...
  const grossFull = fullEarnings + slip.inputs.overtimePaise + slip.reimbursementsPaise;
  const net = slip.netPayablePaise;
  const earningRows = slip.earnings.map((e) => `<tr><td><div class="n">${esc(e.name)}</div>${e.fullPaise !== e.paidPaise ? `<div class="s">Monthly rate: ${inr(e.fullPaise)}</div>` : ''}</td><td class="r">${inr(e.fullPaise)}</td></tr>`).join('');
...
<style>
  * { box-sizing: border-box; }
  ...
  .co { font-size: 26px; font-weight: 600; } .addr { color: #6B7280; margin-top: 4px; font-size: 15px; }
  .for { text-align: right; color: #6B7280; font-size: 15px; } .for b { display: block; color: #222; font-size: 22px; font-weight: 500; margin-top: 4px; }
  ...
  .kv { display: flex; justify-content: space-between; padding: 7px 0; font-size: 15px; } .kv span:first-child { color: #6B7280; }
  .net { border: 1px solid #D1FAE5; border-radius: 12px; overflow: hidden; } .net .top { background: #ECFDF5; padding: 22px 24px; }
  .net .amt { font-size: 32px; font-weight: 600; } .net .cap { color: #047857; margin-top: 4px; font-size: 15px; } .net .days { padding: 12px 24px; }
  ...
  td { padding: 12px 0; vertical-align: top; font-size: 15px; } td.r, th.r { text-align: right; white-space: nowrap; }
  ...
  <div class="head"><div><div class="co">${esc(company.name)}</div>${company.address ? `<div class="addr">${esc(company.address)}</div>` : ''}</div><div class="for">Payslip for the month<b>${esc(monthLabel(slip.period))}</b></div></div>
  <div class="summary">
    <div><div class="label">Employee summary</div>
    ...
    <div class="net"><div class="top"><div class="amt">${inr(net)}</div><div class="cap">Total net pay</div></div>
      <div class="days"><div class="kv"><span>Paid days</span><span>${slip.inputs.payableDays}</span></div><div class="kv"><span>Loss of pay days</span><span>${slip.inputs.lopDays}</span></div>${covered > 0 ? `<div class="kv"><span>Covered by paid leave</span><span>${covered}</span></div>` : ''}</div>
    </div>
  </div>
  <div class="cols">
    <table><thead><tr><th>Earnings</th><th class="r">Amount</th></tr></thead><tbody>${earningRows}${otRow}${reimb}<tr class="tot"><td>Gross earnings</td><td class="r">${inr(grossFull)}</td></tr></tbody></table>
    <table><thead><tr><th>Deductions</th><th class="r">Amount</th></tr></thead><tbody>${deductionRows}<tr class="tot"><td>Total deductions</td><td class="r">${inr(totalDeductions)}</td></tr></tbody></table>
  </div>
  <div class="payable"><div><div class="t">Total net payable</div><div class="sub">Gross ${inr(grossFull)} − deductions ${inr(totalDeductions)}</div></div><div class="amt">${inr(net)}</div></div>
